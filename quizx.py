...
def decrypt_a1z26(text: str) -> str:
    ...
    for i in text:
        if i.isdigit() or i == "-":
            ...
            positions.append(current_word)
            current_word = " "
        else:
            positions.append(current_word + i)
            current_word = ""
    ...
    for word in positions:
        if word.replace('-', '').isdigit():
            letters = [capital_letters[int(char) - 1] for char in word.split('-') if char.isdigit()]
            dec += ''.join(letters)
        else:
            dec += word
...
#         if i.isdigit():
#             position = int(i)
#             dec += capital_letters[position - 1]
#         else:
#             dec += i
