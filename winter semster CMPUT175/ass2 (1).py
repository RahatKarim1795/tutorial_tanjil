...
def product_details(product_file):
    ...
    for lines in product_file[1:]:
        ...
        product_id = split_file[0].strip()
        product_detail[product_id] = {
                         'product_name':split_file[1].strip(),
                         'price':float(split_file[2].strip())
                        }
...
def q1(total_sale, product_detail):
    ...
    for id, details in total_sale.items():
        ...
        if transaction_date < compare_date:
            ...
            if details['discount'] == 0:
                ...
                tot_prod_discount_before[position-1] += details['discount']
            ...
            if details['discount'] == 0:
                ...
                tot_prod_discount_after[position-1] += details['discount']
                product_count_after[position-1] += 1
                
    for i in range(0,20):
        if product_count_before[i] > 0:
            avg_prod_discount_before[i] = tot_prod_discount_before[i] / product_count_before[i]
        if product_count_after[i] > 0:
            avg_prod_discount_after[i] = tot_prod_discount_after[i] / product_count_after[i]
    ...
    for id,details in product_detail.items():
        ...
        print(f'{id:<3} | {product_name:<20} | {avg_prod_discount_before[position-1]:.2f} | {avg_prod_discount_after[position-1]:.2f}')
...
#         original_price = int(product_detail[prod_id]['price'])
#         discounted_price = float(original_price - (original_price * int(details['discount'])))
...
#     for i in range(0,7):
#         print(f'|{weekdays[i]:<9} | {weekly_sale_number[i]:<3} | ${weekly_sale_amount[i]:<10.2f}|')
...
def weekly_business(total_sale, product_detail):
    ...
    days_in_jan = []
    for i in range(2,32):
        days_in_jan.append(datetime(2024, 1, i).weekday())
    
    day_count = {}
    for i in range(7):
        day_count[i] = days_in_jan.count(i)
    ...
    for id, details in total_sale.items():
        ...
        # Assuming product_detail stores prices as strings, we need to convert to float
        original_price = float(product_detail[prod_id]['price'])
        
        # Convert discount percentage to a multiplier, assuming it is provided as an integer
        discount_multiplier = (100 - int(details['discount'])) / 100
    ...
    for i in range(0, 7):
        NB_Tr = round(float(weekly_sale_number[i])/day_count[i])
        Turnover = weekly_sale_amount[i]/day_count[i]
        print(f'|{weekdays[i]:<9} | {NB_Tr:<3} | ${Turnover:<10.2f}|')
    ...
    Turnover_dict = {}
    for i in range(0, 7):
        NB_Tr_dict[i] = round(float(weekly_sale_number[i])/day_count[i])
        Turnover_dict[i] = weekly_sale_amount[i]/day_count[i]

    plt.figure(figsize=(10, 5))
    plt.bar(weekdays, list(NB_Tr_dict.values()), color=bar_colors)
    plt.xlabel('Days of the Week')
    ...
    plt.figure(figsize=(10, 5))
    plt.bar(weekdays, list(Turnover_dict.values()), color=bar_colors)
    plt.xlabel('Days of the Week')
    plt.ylabel('Turnover')
    plt.title('Turnover per Day')
    plt.show()
...
def most_expensive_day(return_detail,product_detail,total_sale):
    ...
    for id,detail in return_detail.items():
        ...
        reshelf_cost = int(product_detail[prod_id]['price']) * 0.1
    ...
    for i in range(len(returned_products)):
        ...
        total_rs = int(rs * no_of_returns[i])
        print(f'{returned_products[i]:<3} {product_detail[returned_products[i]]['product_name']:>20} {no_of_returns[i]:>3} ${total_rs:>10.2f}')
...
def correlation(total_sale,product_detail,each_product_sale):
    tot_discount = [0] * 20
    avg_discount = [0] * 20
    original_price = [0] * 20
    ...
    for id,detail in total_sale.items():
        prod_id = detail['product_id']
        position = int(prod_id[1:])
        tot_discount[position - 1] += float(detail['discount'])
    
    for i in range(0,20):
        avg_discount[i] = tot_discount[i] / each_product_sale[i]
        prod_id = "P" + str(i+1)
...
def main():
    ...
    print("\nQ6:\n")
    correlation(total_sale,product_detail,each_product_sale)
